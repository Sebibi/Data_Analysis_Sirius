...
void mexFunction(int32_T nlhs, mxArray *plhs[], int32_T nrhs,
                 const mxArray *prhs[])
{
  mexAtExit(&adding_atexit);
  /* Module initialization. */
  adding_initialize();
  /* Dispatch the entry-point. */
  unsafe_adding_mexFunction(nlhs, plhs, nrhs, prhs);
  /* Module termination. */
  adding_terminate();
}
                               ...
                               const mxArray *prhs[2])
{
  ...
  b_prhs[0] = prhs[0];
  b_prhs[1] = prhs[1];
  adding_api(b_prhs, &outputs);
  /* Copy over outputs to the caller. */
  emlrtReturnArrays(1, &plhs[0], &outputs);
}

/*
 * File trailer for _coder_adding_mex.c
 *
 * [EOF]
 */
